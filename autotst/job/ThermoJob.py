from autotst.calculator.gaussian import read_log,write_input,Gaussian,get_HF_ZPE
...
class ThermoJob():
    ...
    def __init__(
            ...
            partition="short", # The partition to run calculations on
        ...
        self.smiles = self.species.smiles[0]

        self.calculator = calculator
        self.orca_log_path = None
    ...
    def calculate_conformer(self, conformer, restart=False):
        """
        ...
        calculator.conformer = conformer
        ...
        if not converged:
            ...
            os.remove(log_path)
            calculator.conformer = conformer
            calculator.settings["convergence"] = ""
            calc = calculator.get_conformer_calc()
    ...
    def calculate_sp(self,conformer,sp_method,restart=False):
        ...
        if str(sp_method) != "orca":
            ...
            calc = Orca(conformer=conformer,directory=os.path.join(
                ...
                conformer.smiles,
                "sp"))
            # label = calc.write_sp_input(write_input=False)
            # restart = True ######    remove this later
            # calc.label = "{}_{}".format(conformer.smiles, sp_method)
            label = calc.label
            log_path = os.path.join(calc.scratch,calc.label + ".log")
            self.orca_log_path = log_path
            slurm_path = os.path.join(calc.scratch,calc.label + ".slurm.log")
    ...
    def _calculate_fod(self,conformer,method_name):
        ...
        if not attempted or not complete:
            ...
            subprocess.Popen(
                """sbatch --exclude=c5003,c3040,c3041 --job-name="{0}" --output="{0}.log" --error="{0}.slurm.log" -p express -N 1 -n 4 -t 10:00 --mem=5GB $AUTOTST/autotst/job/orca_submit.sh""".format(
...
################## rotor methods #############################################
    def calculate_rotors(self, conformer, steps=24, step_size=15.0, attempts=0):
        ...
        while not done:
            for index,label in list(complete.keys()):
                ...
                if not all([completed, converged]) and recalc[label] is False:
                    ase_calculator = self.calculator.get_rotor_calc(index, steps, step_size, convergence='')
        ...
        if conformer_error:
            ...
            logging.info("removing {}".format(log_path))
            os.remove(log_path)
            ...
            conformer._ase_molecule = atoms
            conformer.update_coords_from("ase")
            conformer.index = 'rotor'
            
            if not self.calculate_conformer(conformer,restart=True):
                assert False
            ...
            label = "{}_{}_{}_optfreq".format(self.smiles,conformer.index,self.method_name)
            log_path = os.path.join(new_scratch,label+ '.log')
            ...
            new_label = "{}_{}_optfreq".format(self.smiles,self.method_name)
            dest = os.path.join(self.directory,"species",self.method_name,conformer.smiles,new_label+".log")
            copyfile(log_path,dest)
    ...
    def verify_rotor(self, conformer, label, steps=24, step_size=15.0):
        """
        A method that will 
        """
    ...
    def check_rotor_lowest_conf(self, parser, tol=0.01036):
        ...
        if abs(minimum_energy - min(starting_energy, final_energy)) > tol:
            ...
            atomcoords = parser.converged_geometries[energies.index(minimum_energy)]
            atoms = []
    ...
    def check_rotor_isomorphic(self,parser):
    ...
    def determine_rotor_symmetry(self,parser):
        ...
        energies = [parser.scfenergies[i] for i,status in enumerate(parser.optstatus) if
                            status in (2,4,5)]
        energies -= np.min(energies)
        energies *= constants.E_h * constants.Na
        ...
        if len(peaks) != len(valleys):
            logging.warning('Rotor does not have the same number'
        ...
        if symmetry not in [1, 2, 3]:
            logging.warning('Determined symmetry number {0} for rotor; '
                            'you should make sure this makes sense'.format(symmetry))
        else:
            logging.info('Determined a symmetry number of {0} for rotor'
                        ' based on the {1}.'.format(symmetry, reason))
        return symmetry
    ...
    def calculate_species(self, 
        ...
        if options['rotors']:
            ...
            if options["run_rotors"] is True:
                self.calculate_rotors(
                    conformer, steps=24, step_size=15.0)
        ...
        if options["run_sp"]:
            ...
            if 'G4' in single_point_methods:
                ...
                # self.calculator.settings["method"] = "B3LYP"
                # self.calculator.settings["basis"] = "6-31G(2df,p)"
                # self.calculator.settings["basis"] = "6-311+G(d,p)"
                # self.calculator.settings["dispersion"] = None
        ...
        if options['run_arkane_sp']:
            ...
            for sp_method in single_point_methods:
                ...
                if sp_method == 'orca':
                    orca_calc = Orca()
                    orca_path = os.path.join(sp_dir,smiles+'_ORCA.log')
                    energy = orca_calc.read_energy(orca_path)
                ...
                if len(energy_differences) > 0:
                    if max(energy_differences) > 4.14:
                        assert False
